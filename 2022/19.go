...
func Day19(data string) {
	...
	for _, b := range blueprints[0:num_blueprints] {
		max_ore := b.ore_ore
		...
		if max_ore > max_robots {
			max_ore = max_robots
		}
		max_clay := b.obsidian_clay
		if max_clay > max_robots {
			max_clay = max_robots
		}
		max_obsidian := b.geode_obsidian
		if max_obsidian > max_robots {
			max_obsidian = max_robots
		}
		...
		for len(queue) > 0 {
			...
			optimistic := geodes_found_optimistic(b, p, minutes, false)

			if len(p) <= len(executed_plan) && optimistic >= max_geodes {
...
func geodes_found_optimistic(b *blueprint, p plan, minutes int, print bool) int {
	if print {
		fmt.Print("Blueprint ", b)
	}
	...
	for i := 0; i < minutes; i++ {
		...
		if next_choice == ore {
			...
			if (b.geode_ore <= ore_found && b.geode_obsidian <= obsidian_found) || len(p) == 0 {
				ore_found -= b.geode_ore
				obsidian_found -= b.geode_obsidian
				new_geode_robots++
				if next_choice == geode {
