...
const getInputOutput = (arr) => arr.map((line) => line.split(' | ').map((val) => val.split(' ')))
const getOutputs = (inOuts) => inOuts.map(([_, out]) => out).flat()
console.log(
    getOutputs(getInputOutput(notebook)).filter(
        (output) => output.length === 2 || output.length === 3 || output.length === 4 || output.length === 7
    ).length
)

const getNumbers = (inOut) => inOut.map(getNumber)

const getDigitF =
    ([[a, b, c, d, e, f, g], [one, two, three, four, five, six, seven, eight, nine]]) =>
    (out) => {
        if (out.length == 2) {
            return '1'
        }
...
const getNumber = ([ins, outs]) => {
    const places = ['a', 'b', 'c', 'd', 'e', 'f', 'g']
    let a, b, c, d, e, f, g
    const one = ins.find((i) => i.length === 2)
    const seven = ins.find((i) => i.length === 3)
    const four = ins.find((i) => i.length === 4)
    const eight = ins.find((i) => i.length === 7)
    const zeroSixNine = ins.filter((i) => i.length === 6)
    const twoThreeFive = ins.filter((i) => i.length === 5)
    a = seven.replace(new RegExp(`[${one}]`, 'g'), '')
    const six = zeroSixNine.find((digit) => !one.split('').every((char) => digit.includes(char)))
    f = one.split('').find((char) => six.includes(char))
    c = one.split('').find((char) => char !== f)
    bE = places.filter((place) => twoThreeFive.filter((digit) => digit.includes(place)).length === 1)
    const three = twoThreeFive.find((digit) => bE.every((char) => !digit.includes(char)))
    const twoFive = twoThreeFive.filter((digit) => digit !== three)
    const two = twoFive.find((digit) => digit.includes(c))
    const five = twoThreeFive.find((digit) => digit !== two && digit !== three)
    e = two.split('').find((char) => char !== c && !five.includes(char))
    b = five.split('').find((char) => char !== f && !two.includes(char))
    d = four.split('').find((char) => ![b, c, f].includes(char))
    const zero = zeroSixNine.find((digit) => !digit.includes(d))
    e = two.split('').find((char) => !three.includes(char) && char !== f)
    g = places.find((place) => ![a, b, c, d, e, f].includes(place))
    const nine = zeroSixNine.find((digit) => digit !== zero && digit !== six)
    const getDigit = getDigitF([
        [a, b, c, d, e, f, g],
        [one, two, three, four, five, six, seven, eight, nine],
    ])

    const num = outs.map((out) => getDigit(out)).join('')
    return parseInt(num)
}
console.log(getNumbers(getInputOutput(notebook)).reduce((p, n) => p + n, 0))
// console.log(getNumbers(getInputOutput(['acedgfb cdfbe gcdfa fbcad dab cefabd cdfgeb eafb cagedb ab | cdfeb fcadb cdfeb cdbaf'])))
